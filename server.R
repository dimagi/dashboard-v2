...
function(input, output, session) {
  ...
  output$map <- renderLeaflet({
    leaflet( data = geo_data()) %>% addTiles() %>% setView(-1.53, 12.36, zoom = 7, options = list(animate = T)) %>% addMarkers(~longitude, ~latitude, popup = ~popup, layerId = ~mobile_user, group = ~district)
  })
